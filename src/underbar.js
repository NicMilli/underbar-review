(function() {
  ...
  _.last = function(array, n) {
    ...
    return n === undefined ? array.slice(-1)[0] : array.slice(n - 1);
  };
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    _.each(array, function(value) {
      var workingValue = iterator ? iterator(value) : value;
      if (!existing.includes(workingValue)) {
        existing.push(workingValue);
        results.push(value);
      }
    });
    return results;
  ...
  _.memoize = function(func) {
    ...
    return function() {
      let keys = Object.keys(alreadyCalled);
      let stringArgs = JSON.stringify(arguments);
      if (keys.includes(stringArgs)) {
        return alreadyCalled[stringArgs];
      }

      return alreadyCalled[stringArgs] = func.apply(this, arguments);
    };
  };
  ...
  _.shuffle = function(array) {
    ...
    while (returnArray.length < array.length) {
      returnArray.push(args.splice(Math.random() * args.length, 1)[0]);
    }
    return returnArray;
  };
  ...
  _.invoke = function(collection, functionOrKey, args) {
    ...
    _.each(collection, (value) => {
      typeof(functionOrKey) === 'function'
        ...
        : result.push(value[functionOrKey](args));
    });
    return result;
  };
  ...
  _.sortBy = function(collection, iterator) {
    ...
    var right = [];
    var newArray = [];
    var pivot = iterator(collection.pop());
    var length = collection.length;
    for (var i = 0; i < length; i++) {
      if (iterator(collection[i]) <= pivot) {
